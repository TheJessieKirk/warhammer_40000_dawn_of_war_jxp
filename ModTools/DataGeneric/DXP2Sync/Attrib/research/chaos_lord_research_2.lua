----------------------------------------
-- File: 'research\chaos_lord_research_2.lua'
-- Created by: AttributeEditor v2.0
-- Note: Do NOT edit by hand!
-- (c) 2001 Relic Entertainment Inc.

GameData = Inherit([[research\chaos_lord_research_1.lua]])
MetaData = InheritMeta([[research\chaos_lord_research_1.lua]])

GameData["requirements"]["required_1"] = Reference([[requirements\required_research.lua]])
GameData["requirements"]["required_1"]["research_name"] = "chaos_lord_research_1"
GameData["requirements"]["required_10"] = Reference([[requirements\required_research.lua]])
GameData["requirements"]["required_10"]["is_display_requirement"] = true
GameData["requirements"]["required_10"]["research_name"] = "chaos_lord_research_1"
GameData["requirements"]["required_2"] = Reference([[requirements\global_required_addon.lua]])
GameData["requirements"]["required_2"]["global_addon_name"] = "chaos_hq_addon_2"
GameData["time_cost"]["cost"]["power"] = 25.00000
GameData["time_cost"]["cost"]["requisition"] = 100.00000
GameData["time_cost"]["time_seconds"] = 80.00000
GameData["ui_hotkey_name"] = "chaos_lord_research_2"
GameData["ui_info"]["help_text_id"] = "$95561"
GameData["ui_info"]["help_text_list"]["text_01"] = "$95562"
GameData["ui_info"]["help_text_list"]["text_02"] = "$584150"
GameData["ui_info"]["help_text_list"]["text_03"] = "$0"
GameData["ui_info"]["icon_name"] = "chaos_icons/chaoslordability2_icon"
GameData["ui_info"]["screen_name_id"] = "$95560"


MetaData["$METADATATAG"] = {desc = [[]], type = 7, category = [[]], dispval = [[]], isLocked = false, rangeStart = 584150, rangeEnd = 584199, }
MetaData["$METACOLOURTAG"] = 
{

}
