----------------------------------------
-- File: 'research\ork_research_grot_infiltrate.lua'
-- Created by: AttributeEditor v2.0
-- Note: Do NOT edit by hand!
-- (c) 2001 Relic Entertainment Inc.

GameData = Inherit([[research\ork_research.lua]])
MetaData = InheritMeta([[research\ork_research.lua]])

GameData["time_cost"]["cost"]["requisition"] = 50.00000
GameData["time_cost"]["time_seconds"] = 25.00000
GameData["ui_hotkey_name"] = "ork_research_grot_infiltration"
GameData["ui_index_hint"] = 5.00000
GameData["ui_info"]["help_text_id"] = "$96091"
GameData["ui_info"]["help_text_list"]["text_01"] = "$96092"
GameData["ui_info"]["help_text_list"]["text_02"] = "$95453"
GameData["ui_info"]["help_text_list"]["text_03"] = "$95454"
GameData["ui_info"]["icon_name"] = "ork_icons/research_grotinfiltrate_icon"
GameData["ui_info"]["screen_name_id"] = "$96090"


MetaData["$METADATATAG"] = {desc = [[]], type = 7, category = [[]], dispval = [[]], isLocked = false, rangeStart = 582750, rangeEnd = 582799, }
MetaData["$METACOLOURTAG"] = 
{

}
