----------------------------------------
-- File: 'research\eldar_ranger_infiltration_research.lua'
-- Created by: AttributeEditor v2.0
-- Note: Do NOT edit by hand!
-- (c) 2001 Relic Entertainment Inc.

GameData = Inherit([[research\eldar_research.nil]])
MetaData = InheritMeta([[research\eldar_research.nil]])

GameData["time_cost"]["cost"]["power"] = 50.00000
GameData["time_cost"]["cost"]["requisition"] = 50.00000
GameData["time_cost"]["time_seconds"] = 25.00000
GameData["ui_hotkey_name"] = "eldar_ranger_infiltration_research"
GameData["ui_index_hint"] = 9.00000
GameData["ui_info"]["help_text_id"] = "$96411"
GameData["ui_info"]["help_text_list"]["text_01"] = "$96412"
GameData["ui_info"]["help_text_list"]["text_02"] = "$95453"
GameData["ui_info"]["help_text_list"]["text_03"] = "$95454"
GameData["ui_info"]["icon_name"] = "eldar_icons/ranger_infiltrate_icon"
GameData["ui_info"]["screen_name_id"] = "$96410"


MetaData["$METADATATAG"] = {desc = [[]], type = 7, category = [[]], dispval = [[]], isLocked = false, rangeStart = 582700, rangeEnd = 582749, }
MetaData["$METACOLOURTAG"] = 
{

}
